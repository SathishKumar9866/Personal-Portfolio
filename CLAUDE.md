...
- **This repo is the label `repo:personal-portfolio`.** Every work item you create must carry it.
...
2. **A found bug outside the current task's scope gets filed, not silently left.**
   File it in `Backlog` with `repo:personal-portfolio`, say in your reply that you filed it.
